...
function updateChart(years, initialAmount, initialEntryFeesRate, annualInterestRate, annualManagementFeesRate, monthlyEntryFeesRate, indexationRate, monthlyContribution, targetAmount, totalContributionsValue, totalInitialEntryFees, totalMonthlyEntryFees, totalManagementFees) {
    ...
    // Ajout des frais au cumul des frais
    let cumulativeFees = totalInitialEntryFees;
    const feesData = [cumulativeFees]; // Frais d'entrée initiaux à l'année 0

    // Calcul du cumul des frais
    let monthlyCumulativeFees = cumulativeFees;
    ...
    for (let year = 1; year <= years; year++) {
        ...
        for (let month = 1; month <= 12; month++) {
            ...
            const currentMonthFees = managementFees + monthlyEntryFees;
            yearlyFees += currentMonthFees;

        }
        monthlyCumulativeFees += yearlyFees;
        feesData.push(monthlyCumulativeFees);
    }
